...
const createPost = async(req,res)=>{
    try{
        ...
        const recentPosts = await Post.countDocuments({author:req.user.id,createdAt:{$gte:oneHourBefore}}) //gte = greater than or equal to and mongoDb has BSON Date type which also stores in milliseconds so it requires milliseconds not string like ISO String "2026-08-01T10:00:00Z"
        ...
        await post.save();
        await post.populate("author","name bio avatar")
...
const getFeed = async(req,res)=>{
    try{
        ...
        const limit =5;
        const skip = (page-1)*limit;                                                            //note mongoose always send _id unless you write -_id
        const posts = await Post.find({}).sort({createdAt:-1}).skip(skip).limit(limit).populate("author", "name bio avatar").lean(); //lean gives plain js object but with tradeoff not save,populate etc method is not available afterward.
        const postsWithLiked = posts.map((post)=>{ //note individual post is plain js now 
               return {...post,liked:req.user?post.likes.some((id)=>(id.toString()===req.user.id)):false} // some will check if it will find true and break when it finds or after end return false if user exists.
        })
        const total = await Post.countDocuments({});
        return res.status(200).json({posts:postsWithLiked,page,total,hasMore:skip+posts.length<total});
    }
    catch(err){
        return res.status(500).json({error:err.message});
    }
